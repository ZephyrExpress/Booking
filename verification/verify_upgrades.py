...
def run():
    with sync_playwright() as p:
            ...
            window.callApi = async function(data) {
                ...
                if (data.action === 'getTempEntries') {
                    return {
                        result: "success",
                        data: [
                            { awb: "TEMP-TEST", date: "2023-10-27", client: "Client A", weight: "10.5", boxes: [{weight:5}, {weight:5.5}] }
                        ]
                    };
                }
        ...
        # Take screenshot of Temp Entry
        page.screenshot(path="verification/temp_entry_v2.png")

        # Test Auto Inbound Modal
        page.click("button[onclick=\"switchInboundMode('Auto')\"]")
        page.wait_for_selector("#view_auto_inbound", state="visible")

        # Type network no
        page.fill("#auto_inbound_input", "12345")

        # Click Automation Done
        page.click("button[onclick^='handleAutomationScan']")

        # Wait for Modal
        page.wait_for_selector("#forceAssignModal", state="visible")

        # Screenshot Modal
        page.screenshot(path="verification/force_assign_modal.png")
