...
def screenshot_assignment_error_fix():
    cwd = os.getcwd()
    ...
    with sync_playwright() as p:
        ...
        # MOCK callApi to prevent error
        page.evaluate("""
            window.callApi = async function(data) {
                if (!data) return { result: 'error', message: 'No data' };
                if (data.action === 'bulkAssign') {
                    return { result: 'success', message: 'Assigned' };
                }
        ...
        time.sleep(0.5)
        page.screenshot(path="verification/bulk_assign_fixed.png")
        print("Screenshot saved: bulk_assign_fixed.png")

        browser.close()

if __name__ == "__main__":
    screenshot_assignment_error_fix()
